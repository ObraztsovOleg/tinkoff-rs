...
pub enum PointRequest {
    ...
    InsiderDeals(GetInsiderDealsRequest)
}
...
pub enum PointResponse {
    ...
    InsiderDeals(GetInsiderDealsResponse)
}
...
impl PointRequest {
    pub async fn request(self, service: Investment) -> TSResult<PointResponse> {
        match self {
            ...
            PointRequest::InsiderDeals(req) => Ok(
                        PointResponse::InsiderDeals(
                            InstrumentsServiceClient::from(service.into_inner()).get_insider_deals(req).await?.into_inner()
                        )
                    ),
